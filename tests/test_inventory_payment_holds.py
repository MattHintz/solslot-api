...
async def test_v11_migration_preserves_prior_authorizations_and_absent_capability_stays_disabled(tmp_path,monkeypatch):
    ...
    try:
        ...
        c.ledgers[0]._conn.executescript('DROP TABLE inventory_payment_holds; DROP TABLE base_inventory_holds; DROP TABLE base_lifecycle_observations; DROP TABLE base_lifecycle_terminals; DROP TABLE base_inventory_hold_generations; DROP TABLE voucher_series_phase_retry_signatures; DROP TABLE voucher_phase_expiry_observations; PRAGMA user_version=11;')
        c.ledgers[0].close();c.ledgers[0]=ValidatorLedger(tmp_path/'extension-signer-0.db')
