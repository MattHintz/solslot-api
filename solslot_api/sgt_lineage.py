...
async def confirmed_sgt_lineage(*, provider, coin: Coin, expected_inner_hash: bytes32,
    expected_tail_hash: bytes32) -> LineageProof:
...
"""
    ...
    if coin.puzzle_hash != expected_full_hash:
        raise ValueError('SGT receipt does not belong to the expected vault')
    ...
    if (parent is None or child != coin or parent.name() != coin.parent_coin_info or height <= 0
        ...
        raise ValueError('SGT receipt lineage is not a confirmed unspent atomic transfer')
    raw = await provider.get_puzzle_and_solution(parent_id, height)
    if not isinstance(raw, Mapping):
        raise ValueError('SGT parent spend is unavailable')
    ...
    if (reveal.get_tree_hash() != parent.puzzle_hash or mod != CAT_MOD or len(args) != 3
        ...
        raise ValueError('SGT parent reveal or asset identity is inconsistent')
    if coin not in compute_additions(make_spend(parent, reveal, solution)):
        raise ValueError('SGT parent spend did not create the selected receipt')
